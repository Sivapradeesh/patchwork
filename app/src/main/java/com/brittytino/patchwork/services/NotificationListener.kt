...
class NotificationListener : NotificationListenerService() {
    ...
    companion object {
        const val ACTION_LIKE_CURRENT_SONG = "com.brittytino.patchwork.ACTION_LIKE_CURRENT_SONG"
        const val ACTION_REQUEST_AMBIENT_GLANCE =
            "com.brittytino.patchwork.ACTION_REQUEST_AMBIENT_GLANCE"
    }
    ...
    private fun discoverMapsChannel(channelId: String?, userHandle: android.os.UserHandle) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            try {
                ...
                val discoveredChannels: MutableList<com.brittytino.patchwork.domain.model.MapsChannel> =
                    if (discoveredJson != null) {
                        try {
                            gson.fromJson(discoveredJson, Array<com.brittytino.patchwork.domain.model.MapsChannel>::class.java).toMutableList()
                        } catch (_: Exception) {
                            mutableListOf()
                        }
                    } else mutableListOf()
                ...
                if (discoveredChannels.none { it.id == channelId }) {
                    ...
                    discoveredChannels.add(
                        com.brittytino.patchwork.domain.model.MapsChannel(
                            channelId,
                            name
                        )
                    )
    ...
    private fun discoverSystemChannel(
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            try {
                ...
                val gson = com.google.gson.Gson()
                val discoveredChannels: MutableList<com.brittytino.patchwork.domain.model.SnoozeChannel> =
                    if (discoveredJson != null) {
                        try {
                            gson.fromJson(discoveredJson, Array<com.brittytino.patchwork.domain.model.SnoozeChannel>::class.java).toMutableList()
                        } catch (_: Exception) {
                            mutableListOf()
                        }
                    } else mutableListOf()
                ...
                if (discoveredChannels.none { it.id == channelId }) {
                    ...
                    discoveredChannels.add(
                        com.brittytino.patchwork.domain.model.SnoozeChannel(
                            channelId,
                            finalName
                        )
                    )
    ...
    private fun triggerAmbientGlance(
        ...
        if (isEnabled) {
            ...
            val isDreaming = com.brittytino.patchwork.services.dreams.AmbientDreamService.isDreaming
                ...
                if (hasPermissions) {
                    ...
                    if (appSelected) {
                        ...
                        val styleName = prefs.getString(
                            "edge_lighting_style",
                            com.brittytino.patchwork.domain.model.NotificationLightingStyle.STROKE.name
                        )
    ...
    private fun isAppSelectedForNotificationGlance(packageName: String): Boolean {
        try {
            ...
            val selectedApps: List<com.brittytino.patchwork.domain.model.AppSelection> =
                com.google.gson.Gson().fromJson(json, Array<com.brittytino.patchwork.domain.model.AppSelection>::class.java).toList()
    ...
    private fun isAppSelectedForNotificationLighting(packageName: String): Boolean {
        try {
            ...
            val gson = com.google.gson.Gson()
            val selectedApps: List<com.brittytino.patchwork.domain.model.AppSelection> =
                gson.fromJson(json, Array<com.brittytino.patchwork.domain.model.AppSelection>::class.java).toList()
