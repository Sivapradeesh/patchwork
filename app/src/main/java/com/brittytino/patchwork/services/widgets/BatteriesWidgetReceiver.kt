    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (intent.action == Intent.ACTION_POWER_CONNECTED ||
            ...
            kotlinx.coroutines.MainScope().launch {
                ...
                val repository =
                    com.brittytino.patchwork.data.repository.SettingsRepository(context)
                val hasPerm =
                    com.brittytino.patchwork.utils.PermissionUtils.hasBluetoothPermission(context)

                val isEnabled = repository.isBluetoothDevicesEnabled()
                val bluetoothDevices = if (isEnabled && hasPerm) {
                    com.brittytino.patchwork.utils.BluetoothBatteryUtils.getPairedDevicesBattery(
                        context
                    )
                } else {
                    emptyList()
            ...
            try {
                val requestIntent =
                    Intent("com.brittytino.airsync.action.REQUEST_MAC_BATTERY").apply {
