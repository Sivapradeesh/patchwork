...
class AmbientGlanceHandler(
    ...
    fun checkAndShowOnScreenOff() {
        val prefs = service.getSharedPreferences(
            com.brittytino.patchwork.data.repository.SettingsRepository.PREFS_NAME,
            Context.MODE_PRIVATE
        )
        val isEnabled = prefs.getBoolean(
            com.brittytino.patchwork.data.repository.SettingsRepository.KEY_AMBIENT_MUSIC_GLANCE_ENABLED,
            false
        )
        val isDocked = prefs.getBoolean(
            com.brittytino.patchwork.data.repository.SettingsRepository.KEY_AMBIENT_MUSIC_GLANCE_DOCKED_MODE,
            false
        )

        if (isEnabled && isDocked) {
            val intent = Intent("com.brittytino.patchwork.ACTION_REQUEST_AMBIENT_GLANCE").apply {
            ...
            try {
                ...
                val componentName = android.content.ComponentName(
                    service,
                    com.brittytino.patchwork.services.NotificationListener::class.java
                )
